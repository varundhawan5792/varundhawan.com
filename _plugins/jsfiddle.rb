    ...
    class JsFiddle < Liquid::Tag
      def initialize(tag_name, markup, tokens)
        if /(?<fiddle>\w+\/?\d?)(?:\s+(?<sequence>[\w,]+))?(?:\s+(?<embedWay>\w+))?(?:\s+(?<skin>\w+))?(?:\s+(?<height>\w+))?(?:\s+(?<width>\w+))?/ =~ markup
          ...
          @embedWay = (embedWay unless embedWay == 'default') || 'iframe'
          @skin     = (skin unless skin == 'default') || ''
          @width    = width || '100%'
          @height   = height || '300px'
        end
      end
      ...
      def render(context)
        if @fiddle
          if @embedWay == 'iframe'
              "<iframe class=\"jsfiddle-embed\" style=\"width: #{@width}; height: #{@height}\" src=\"//jsfiddle.net/#{@fiddle}/embedded/#{@sequence}/#{@skin}/\" allowfullscreen=\"allowfullscreen\" allowpaymentrequest frameborder=\"0\"></iframe>"
          else
